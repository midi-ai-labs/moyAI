...
export function renderDeviceExecution(local: DeviceNetworkPresentation, shared?: SharedWorkPresentation): string {
  ...
  const folderRows = p?.projects.filter(row => row.can_execute).map(row => {
    const needsReselection = Boolean(p.directory && row.environment_id && !row.directory
      && (row.preparation_state === "ready" || row.preparation_state === "failed"));
    const description = row.directory ? `作業フォルダー: ${esc(row.directory)}`
      : !p.directory ? "上の「1. このPCの実行許可」を保存すると選べます。"
        : !row.environment_id ? "Hubでこのプロジェクトの実行場所を準備しています。"
          : needsReselection ? "このプロジェクトの作業フォルダーを選び直してください。選ぶまで、このPCではこのプロジェクトの新しい仕事を実行できません。"
            : "このプロジェクトの作業フォルダーを選んでください。";
    return `<div class="device-project-folder"><strong>${esc(row.label)}</strong><p>${description}</p>${p.directory && row.error ? `<p class="hub-feedback" data-error="true">${esc(row.error)}</p>` : ""}${row.environment_id ? `<button data-action="bind-project-folder" data-value="${esc(row.id)}" ${projectFolderBindingEnabled(local, row.id) ? "" : "disabled"}>${needsReselection ? "作業フォルダーを選び直す" : row.directory ? "作業フォルダーを変更" : "作業フォルダーを選ぶ"}</button>` : ""}</div>`;
  }).join("") ?? "";
