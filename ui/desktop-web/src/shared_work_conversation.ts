...
export function renderHubConversation(local: SharedWorkPresentation, approvalMarkup: string): string {
  ...
  return `<main class="shared-work" tabindex="-1" data-shared-owner="${esc(owner)}" aria-labelledby="shared-heading">
    <header data-shared-region="account" class="shared-header"><div><h1 id="shared-heading">${esc(project?.label ?? "Hubのプロジェクト")}</h1><p>${person ? `${esc(person.display_name)}${project ? " · プロジェクトの参加者で会話と成果を共有します" : " · 管理者が割り当てると、左のプロジェクト一覧に表示されます"}` : "管理者がこのPCの参加と用途を設定すると、利用できます。"}</p></div><div class="shared-actions">${project ? button("new-conversation", "新しいチャット", "", busy) : ""}<details data-details-key="hub-project-account"><summary>このPCの接続</summary><p>${esc(p?.hub_url)}</p>${person?.administrator ? button("open-management", "Hubの管理画面を開く", "", busy) : ""}<button data-action="show-hub">接続・このPCの設定</button></details></div></header>
    ...
    ${!person ? `<section data-shared-region="connection" class="shared-card"><h2>${p?.connected ? "このPCの利用登録を確認" : "このPCをHubに接続"}</h2>${!p?.connected ? `<p>管理者から受け取った接続ファイル（.moyai-join または .toml）を読み込み、このPCの参加を申請してください。</p>${button("import", "Hubの設定ファイルを読み込む", "", busy)}${button("reconnect", "再接続", "", busy)}` : `<p>Hubに接続しています。管理者がこのPCを承認し、プロジェクトの操作PCに指定すると、そのまま利用できます。</p>${button("refresh", "利用できるプロジェクトを確認", "", busy)}`}</section>${renderSharedWorkOnboarding(local)}` : project ? `<div class="shared-columns"><div class="shared-conversation-column">
      ${approvalMarkup}
      <section data-shared-region="detail" data-shared-record-owner="${esc(recordOwner)}" class="shared-card shared-conversation-record">${detail ? `<h2>${esc(detail.title)}</h2><p>${esc(workStateLabel(detail.state))}${selectedJob ? ` · ${esc(selectedJob.requestor.display_name)} → ${esc(selectedJob.device_label ?? selectedJob.environment_label)}` : ""}</p><div data-shared-runner-observation>${contact(detail.runner_contact)}</div>${detail.wait_reason ? `<p>${esc(detail.wait_reason)}</p>` : ""}${detail.uncertainty_reason ? `<p class="shared-error">${esc(detail.uncertainty_reason)}</p>` : ""}<div class="markdown-body shared-user-message">${renderMarkdown(typeof detail.input === "object" && detail.input !== null && "prompt" in detail.input ? String(detail.input.prompt) : "")}</div>${renderWorkResult(detail.result, recordOwner, detail.can_continue)}${detail.start_before_ms != null ? `<details data-details-key="hub-job-deadline"><summary>実行の設定</summary><p>開始期限: ${esc(new Date(detail.start_before_ms).toLocaleString("ja-JP"))}</p></details>` : ""}${selectedJob?.can_cancel ? button("cancel", "実行を停止", detail.id, busy) : ""}` : `<h2>新しいチャット</h2><p>このプロジェクトで行いたいことを入力してください。</p>`}</section>
