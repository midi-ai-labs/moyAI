...
import { createDeviceNetworkUiState, deviceCanJoin, deviceCanReceive, deviceCanSelect, deviceDraftCurrent,
  deviceNetworkError, devicePeerAvailability, devicePeerKey, deviceReceiverConfirmed, deviceTargetKey, visibleDevicePeers,
  type DeviceNetworkPresentation } from "./device_network_state.ts";

const enrollmentText = { unconfigured: "共通設定を読み込んでください", not_enrolled: "端末の参加前", pending: "参加の確認中",
  active: "Hubに参加済み", disconnected: "Hubとの接続が切れています", revoked: "端末の認証が失効しています", error: "接続を確認してください" };
const accessHelp = { default: "確認が必要な操作は実行しません。受入端末の通常の権限境界を使います。",
  auto_review: "確認が必要な操作を受入端末のAIが審査します。追加の人による承認には対応していません。",
...
export function renderDeviceNetwork(input?: DeviceNetworkPresentation): string {
  ...
  return `<div class="device-network-content">
    ...
    <section class="device-network-card"><div class="hub-section-heading"><h3>Hubへの参加</h3><span data-settings-passive="device-network-enrollment" class="device-network-status ${active ? "ready" : "muted"}">${status}</span></div>
      <p class="hub-help" data-settings-passive="device-network-self">このPC: ${escapeHtml(projection?.local_hostname || "確認中")} · ${projection?.display_name ? `登録名: ${escapeHtml(projection.display_name)}` : "登録名はHub管理者が指定します。"}</p>
      ...
      <p class="hub-help">管理者から受け取った共通TOML設定を読み込みます。モデルや手動MCPの既存設定は置き換えません。</p>
      <div data-network-visible="join" ${member ? "hidden" : ""}>
        <label class="hub-field" for="device-network-code">参加コード<input id="device-network-code" class="settings-control" data-network-field="code" type="password" autocomplete="off" data-settings-dom-value placeholder="Hub管理者が発行した、1回限りのコード" ${busy || !projection?.can_join ? "disabled" : ""} /></label>
        <label class="device-network-check"><input id="device-network-join-confirmed" class="settings-control" data-network-field="join_confirmed" type="checkbox" ${local.joinConfirmed ? "checked" : ""} ${busy || !projection?.can_join ? "disabled" : ""} /><span>このHubへ端末を登録し、管理者が定める接続許可を使うことを確認しました。</span></label>
        <button id="device-network-join" class="hub-primary" data-action="device-network-join" ${deviceCanJoin(local) ? "" : "disabled"}>この端末でHubに参加</button>
      </div>
      <details id="device-network-details" data-details-key="device-network-details"><summary>接続の詳細</summary><div class="device-network-technical" data-settings-passive="device-network-details"><p>Hub: ${escapeHtml(projection?.hub_url || "未設定")}</p><p>端末ID: ${escapeHtml(projection?.device_id ?? "未登録")}</p><p>受付先: ${escapeHtml(receiver?.endpoint ?? "停止中")}</p><p>IP・ポート・証明書は自動設定します。秘密鍵と一時的な接続資格情報は表示しません。</p></div></details>
    </section>
    ...
    <section class="device-network-card" data-network-visible="member" ${member ? "" : "hidden"} aria-labelledby="device-network-peers-title"><div class="hub-section-heading"><h3 id="device-network-peers-title">利用する端末を選ぶ</h3></div><p class="hub-help">Hubが接続を許可した端末だけを利用先にできます。利用ONは送信側の選択です。実際の利用可否は、相手の受付・接続状態と別に表示します。</p>
      ...
      <div class="device-network-peers" data-settings-passive="device-network-peer-list" data-settings-preserve-focused-region>${peers.length ? peers.map(peer => {
        ...
        return `<article class="device-network-peer"><div><strong>${escapeHtml(peer.display_name)}</strong>${peer.name ? `<span>${escapeHtml(peer.name)}</span>` : ""}<p class="device-network-status ${availability.tone}">${availability.label}</p><p class="hub-help">${escapeHtml(peer.reason ? deviceNetworkError(peer.reason) : "相手の端末が公開対象と実行権限を管理します。接続は実際の依頼時に確認します。")}</p><details data-details-key="device-peer-${escapeHtml(key)}"><summary>端末の識別情報</summary><code>${escapeHtml(peer.device_id)} / ${escapeHtml(peer.profile_id)}</code></details></div><button id="device-network-use-${encodeURIComponent(key)}" data-action="device-network-select" data-value="${escapeHtml(key)}" aria-pressed="${peer.selected}" aria-label="${escapeHtml(peer.display_name)} ${escapeHtml(peer.name ?? "")} 利用${peer.selected ? "ON" : "OFF"}" ${deviceCanSelect(local, key) ? "" : "disabled"}>利用 ${peer.selected ? "ON" : "OFF"}</button></article>`;
