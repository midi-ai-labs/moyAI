...
const stateLabels: Record<string, string> = { preparing: "受付を確認中", accepted: "受付済み", running: "実行中", cancelling: "停止処理中", completed: "完了", failed: "失敗", interrupted: "中断", unknown: "状態未確認" };
...
export function renderDeviceNetworkJobs(local: DeviceNetworkPresentation): string {
  const outgoing = local.jobs.outgoing.map(job => ({ key: `outgoing:${job.reference_id}`, title: "この端末からの依頼",
    path: job.device_path, state: job.state, stop: stopLabels[job.stop_status], result: job.result, root: job.root_task_id }));
  const incoming = local.jobs.incoming.filter(job => job.profile_id === local.projection?.receiver.profile_id).map(job => ({ key: `incoming:${job.job_id}`,
    ...
    root: job.network?.root_task_id ?? job.parent.task_id }));
