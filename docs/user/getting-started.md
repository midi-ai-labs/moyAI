...
2026-07-21 時点のv1.0.0 release向け最小手順。canonical runtime/storage、Codex型Desktop履歴、3種類のpermission mode、Windows workspace sandbox、Responses transportを含む。正確なversionと機能は利用するrelease packageと製品READMEを確認する。
...
次tool、turn終了、verification、compactionを決めることはない。fresh proactive root turnだけは、
正常完了した`update_plan`を直近blockerのroot-local ownership判断として使うが、host-authored plannerや
task DAGは作らない。内部のdurableなPlan mode contractはmutation toolだけを隠して`update_plan`を保持するが、
現時点でCLI/TUI/Desktopにmode selectorはない。
...
rolling timeoutで、どちらも既定値は600,000ms。生成全体の時間上限ではない。必要な場合はconfigまたは
...
- `proactive`: 品質、待ち時間、context効率に有効なbounded並列作業またはcontextを分離した逐次handoffをmodelが判断して委譲できる。
- fresh proactive root turnでは、最初のtool surfaceを`spawn_agent`と`update_plan`に限定する。
  `spawn_agent`の正常完了後は、rootを追加spawn / message / wait / list / interruptと`update_plan`だけの
  collaboration surfaceへ移し、delegated inputをrootのworkspace toolで重複処理しない。rootが明確に分離した
  非重複blockerを持つ場合だけ、`update_plan`の正常完了でroot-local ownershipへ移り、次requestから通常surfaceへ戻る。
  失敗・拒否ではownershipを変えない。`spawn_agent`と同じmodel responseへworkspace toolを混在させても、
  委譲前に広告済みのsiblingを実行しない。child、`explicit_request_only`、Plan mode、tool非対応model、
  空prompt continuationには適用せず、compaction後も同じrun内の判断を維持する。
- rootは目的、制約、短い進捗、統合、最終回答を保持し、分担内容と件数は現在のevidenceからmodelが
  決める。固定stageはない。childは明確なscopeと一つの検証可能な結果を所有し、書き込む場合だけmutation
  ownerを1体に限定する。依存handoffは同時実行を必要としない。proactive modeでは`fork_turns`を明示し、
  自己完結したtask packetには`"none"`、親contextが必要な場合だけ`"all"`を使う。並列writeはtargetを
  分離し、同じtargetにはwriterを1体だけ置く。materialな変更は別のread-only verifierで確認できる。
...
`ResolvedTurnConfig`を固定する。完全値をPartialへ逆変換せず、後続stepでbase configを再mergeしない。permission decisionだけは各判断直前にdurable root-session modeを読み、ほかのturn configや`RunConfigSnapshot`を変更しない。`UserTurn` / `SteerTurn`を中間wrapperなしで
...
- model policyの90% working targetへ達すると、moyAIのAutomatic compactionは固定item件数ではなくresponse bundle / call-output semantic unitを選ぶ。provider報告total usageがある場合はdurable turn terminalから復元し、そのmodel response後のlocal itemだけをCodexと同じ粗いUTF-8 bytes/4で加算する。usageがないかresponse境界を照合できない場合だけfull prepared requestのlocal推定へfallbackし、request diagnosticsは使用sourceを区別する。tool responseが未完了の間はcompactionせず、summary requestはbase instructionsとnativeなDeveloper / User / Assistant / tool順序を保ってCodex checkpoint promptを最後のUser inputへ追加し、toolsとprovider cursorを送らない。最初にfull native requestを送り、typed `context_length_exceeded`の場合だけ最古のprovider-native itemと必要なcall/output対応相手を除いて再試行し、semantic map/reduceは行わない。checkpointは新しいreal User / Steer text inputと委譲turnを開始したcanonical `NEW_TASK`をoriginal orderのまま保守的な20,000 token以内に保持し、境界の一件は中央を切り詰め、prefix付きsummaryを最後のUser inputにする。通常のagent messageとfinal handoffはsummaryへ残し、古いsummaryをanchorまたはsystem instructionへ昇格させない。置換lineageをcanonical historyへcommitして元itemは保持する。cancel、空summary、tool call混入、provider failureではhistoryを変更しない。
