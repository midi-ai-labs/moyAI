...
- `side-chat.quote` はtool-enabled Responses fixtureのMain Turnでcanonical settled Assistant / file-change行を作り、各行のcanonical `stable_history_identity`へ引用actionを束縛する。選択範囲そのものは一行内へ閉じたdeterministic DOM Rangeをpreconditionとし、action activationはpointer clickとreverse-Tab→Enterのbrowser-trusted eventで別々に取得する。各action後にSide draft / pending quoteだけが更新され、Main composerがbyte-identicalに残り、provider generationと`submit_side_chat`が0件であることを先に判定する。Side paneのowner / owner-session scope / as-of append position / non-truncated表示、2回のexact typed quote payloadを持つ`submit_side_chat`、二つ目をpeer closeまでholdしたRunning、`cancel_side_chat` exact 1 / `cancel_run` 0、Main owner不変、共通cleanup / closed SQLite auditを同じexecutionで束ねる。
- `side-chat.session` は実GUIから同一projectにA（青・金曜日）とB（赤・月曜日）の独立sessionを作り、Aを明示選択してSide Chatを設定・送信する。exact `submit_side_chat` 1件とMain submit/cancel 0件、tool-less outboundにAのcanonical User/Assistant unitと正しいowner ID / append fence / source IDだけが含まれること、MainのDOM / canonical履歴と未送信draftが不変であることを判定する。Sideの未送信draftを保存し、BではSide binding / messages / draftが空の正常な未設定状態、Aへ戻ると同じchat ID / history / draftが復元されることを確認する。exact Desktop restart後もAを明示的に開き直し、Sideの保存状態とprovider request総数3件の不変を安定観測する。ローカルscripted providerによるowner分離回帰であり、live LLMの回答品質を評価するcaseではない。
