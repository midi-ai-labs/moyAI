...
全GUI監査の操作・状態別手順は [MANUAL_GUI_COVERAGE.md](MANUAL_GUI_COVERAGE.md) を参照する。scenarioの実行成功、手動の直接操作、画像の目視は別々に記録し、未確認状態を残す。

通常開発では `npm run verify:gui -- --suite smoke` を単体試験と手動確認の間の自動GUI段階として使う。広い回帰、CI、判定と証跡の読み方は [GUI_AUTOMATION.md](GUI_AUTOMATION.md) を参照する。
...
- `hub.offline-reset` は同じ共通ownerで二つの隔離Hubを用意する。実Desktopの参加承認後に旧Hubのネットワークを停止し、リセットの確認・取消・確定、手動AI設定・sessionの帰属・作業ファイルの保持、新しいCAのHubへの再参加と新しい端末鍵を確認する。ローカルRunnerの停止未確認と旧journalの非再実行はfocused Rust testsで別に確認する。手順の存在だけでは実行済みや物理WinA/WinBの成功としない。
...
- native dialogのsemantic actionはexact PID / start / executable / HWND / thread / class / rootを再検証する。adapterごとにUIA root fingerprint＋UIA pattern、または親control/ID＋readback付きWin32 control入力を一回だけ使う。物理keyboardが要件のscenarioはexact foregroundを入力直前・直後に証明できる専用環境でのみ`SendInput`を使い、adapter間の動的fallbackや入力retryを行わない。
...
- self-test（unit / Windows helper）: `npm run test:desktop-e2e-harness`。Windowsでは実machine admission mutexの排他・解放確認を含むため、actual GUI executionのcleanup完了後に直列実行する。GUI操作を含まないことを、GUI試験と同時実行可能という意味にしない。
...
- Hub 接続と確認設定: `hub.browser-enrollment` で実Hubへの参加承認と統一AI欄のMain/Side別モデル確認を行う。`onboarding.win-a-to-win-b` はHubだけでAIを登録し、実行PCにAIのURL・モデルを手入力せずに共有仕事と成果返却まで確認する。旧token入力・Direct/Hub切替画面を前提にした `hub.connection-settings` は通常シナリオ登録から外した。HTTP fixtureとその契約検査は互換用の補助として保持し、現行GUIの成功証拠へ流用しない。
- 退役した手動端末登録フォームの `settings.mcp-peer-controls` も通常登録・suiteから外した。一般HTTP MCP設定は `settings.preferences-config`、Hub参加・共有仕事は上記シナリオ、保存済み旧利用先の削除は `hub.offline-reset` で確認する。regressionでは現行メニュー・パレットから統一設定への到達も操作する。
...
- `permission.guardian-handoff-approve` / `permission.guardian-handoff-abort` は同じscripted providerのGuardian応答を `ask_user` にし、実Tauriのユーザー確認へ移ることを検証する。64行の説明コメント付きshellを表示しても承認・中止ボタンが画面内にあり操作できること、承認前は処理が走らないこと、許可時は対象コマンドによるローカル証跡の追記が正確に1回、中止時は0回で別の要求も出ないことを照合する。既存のtrusted入力、exact Desktop command、provider ledger、共通quiesce・終了後SQLite監査を使い、restart/pressureシナリオの判定は変更しない。
...
- `settings.preferences-config` は `settings.preferences` と同じfactory・設定操作・保存/restart oracleを使うが、native titlebar dragを対象外とする別ID。開始時のscope evidenceと最終cleanup resourceに「native drag未確認」を残す。provider context/Main system prompt、Docling、dirty close/Cancel/discard、保存と再起動復元、全期間implicit HTTP 0は省略しない。元の `settings.preferences` は引き続きexact HWNDの物理drag取得を必須とし、この別IDのPASSで代替しない。foregroundのAccessDeniedなどで物理dragを取得できない環境でも設定機能の検証を独立して行うための分離であり、native input guardやfallbackは変更しない。
...
npm run qualify:desktop-e2e-harness -- --binary target/debug/moyai-desktop.exe --artifact-parent <absolute-task-root> --scenario settings.preferences-config
...
Hubの管理画面はブラウザーへ移行したため、旧Hubネイティブウィンドウを起動・操作するdriverと結合シナリオは削除した。現在は[HubのPlaywright試験](../../../moyAI-Hub/tests/browser/README.md)と、このharnessの `hub.browser-enrollment`、`hub.offline-reset`、`onboarding.win-a-to-win-b` を使用する。
...
`settings.shared-work-continuation` はHub上で操作・実行を許可した1台のDesktopを用い、実Runnerの初回同意、既存のプロジェクトフォルダの選択、通常のチャット欄からの送信、同じ共有会話への追加送信を実GUIで確認する。さらに最新の仕事を実行中に停止し、その依頼を編集して同じ会話へ再送する。利用者に実行PC・仕事の種類・チャット名を入力させないこと、SendがHub jobを作ること、追加Sendと編集後の仕事が同じconversation IDに属することを観測する。scenario configにはHubの `hubBinary` / `hubRepository`、製品 `runnerBinary`、現在sourceから作ったlibtestの絶対path `runnerTestBinary` を指定する。対象は同一Windows上の仮想的な端末試験であり、物理WinBは試験しない。2台の登録・実行・成果確認は `onboarding.win-a-to-win-b` が担当する。
...
`navigation.session-management` は独立workspaceにGit root markerを置き、3つの短いscripted応答を通常のGUI送信で生成し、session検索・archive/復元・session/Quick Chat/project削除の取消と確定を確認する。検索は一致行に加えて現在開いているsessionを保持するcurrent owner契約を確認する。対象IDを持つcommandが1回だけ実行され、取消で下書き・履歴が保持され、project削除で実workspaceファイルを消さないことを比較する。DBから会話を直接作らない。rename（現行GUIに操作なし）・Running中削除・fork/rollback・再起動はこのscenarioの対象外である。

`input.command-palette-insertion` は独立workspaceに `.moyai/commands/` のfixtureを用意し、Ctrl+K→検索→Tab/Enterで `/コマンド名 ` をMain draftのcaretへ一度だけ挿入する。自動送信・別ownerへの挿入がなく、無該当検索では候補が0件となり、Esc取消してdraftを保持することを確認する。command本文の直接挿入やコマンド実行はこの操作の契約ではない。両scenarioの目視はexecutionに紐づく別receiptへ残す。
