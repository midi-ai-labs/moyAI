...
`manual.provider-lm-studio-thinking` と `manual.permission-temp-escalation-lm-studio` もcredential-freeなURLとmodel IDだけを受理する。modelは実行前にLM Studioでload済みにし、scenarioはそのlifecycleを変更しない。
