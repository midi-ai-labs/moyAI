...
<p align="center">
  <strong>ローカルLLM と、クローズドネットワーク用のコーディングエージェント。</strong>
</p>
...
派手なデモよりも、手元の開発作業で日常的に頼れることを重視しています。モデルはローカルに置き、作業の証跡は見える形で残します。あとから「何を読んだのか」「何を変えたのか」「何を検証したのか」を追えるようにするためです。
...
| 閉域環境へ持ち込みやすい | 配布先で npm、Rust toolchain、internet、dev server を要求しません。 |
...
moyAI は、ユーザー共通の config file を 1 つだけ読みます。その上に environment variables と CLI overrides を重ねて適用します。
...
release folder や workspace folder に専用の config file を置く必要はありません。Desktop、TUI、CLI は同じ設定を参照します。
