...
<p align="center">
  ...
  ·
  <a href="https://github.com/midi-ai-labs/moyAI/releases/tag/v0.8.0">release をダウンロード</a>
  ·
...
- **承認を求める**（`default`）、**代理で承認**（`auto_review`）、**フルアクセス**（`full_access`）の3種類のpermission mode。承認を求める/代理で承認は同じdeterministic workspace boundaryを使い、stable file editを含む境界内かつriskのないList/Search/Read/Editを自動許可する。OS sandboxを持たないshell、external/network effect、境界外またはriskありの操作は、承認を求めるではhuman confirmation、代理で承認ではtask agentと分離したtool-less AI Guardianへ送る。Guardianはbounded human previewとは別のcomplete typed action evidenceを判断し、MCPのnormalized full arguments / configured target / credential presenceと、Doclingのexact endpoint / source / effective options / credential presenceをsecret値なしで受け取る。incomplete evidence、deny、invalid output、unavailable、90秒total deadline超過、request/storage failureはhuman promptへfallbackせずdenyへfail closedにする。Guardian requestはcurrent WorldState、current exact committed call、同じresponse内のbounded prior resultsを使い、task generationのsamplingから分離する。フルアクセスはpermission promptを出さないが、stable-handle、path-integrity、workspace-authority、no-clobber等の製品guardを迂回しない。Desktop/TUIは選択したroot-session ownerを保存する。人間確認の「実行せず、指示を変更する」による`ApprovalAborted`、Guardian denyによる`Declined`、外部Stop、運用失敗は別のtyped outcomeとして扱う。commandは現在のユーザー権限で動く
...
v0.8.0には、このsource treeで説明しているcanonical runtime/storage cutover、Codex型planning、
Responses transport、semantic compaction、Desktop interactionの安定化が含まれます。
