...
Hubへの端末参加・タスク受付・他端末の利用先選択を追加しています。共通設定を読み込み、一度だけの
参加コードで登録した後、受付や許可された接続先の利用をONにします。新しいサーバー端末は、
Directモデルを先に設定せずHubの共通設定から開始できます。共通設定に入るのはHub URLと公開CA信頼で、
各端末が固有の秘密鍵を生成します。Desktop側の待受IP・ポート・TLS資格情報・短期認可の手入力は不要です。
この追加機能を実装し、同一Windows上の実Hub/Desktop画面と分離した送受信runtimeで、共通設定からの開始・参加、
temp受付・有向許可・利用先選択・登録名での承認・CPU調査結果の返却を確認しました。確認済みの操作範囲を示すもので、
物理複数PCの受入を意味しません。以前の暫定ビルドには含まれません。
...
別の **MCPを配信** 画面と手動接続先フォームは、URL・token・TLSを個別設定する互換導線として残ります。
読み取りモードのtempは `current_time`、エージェントモードのtempはタスク実行です。手動配信には独自の
起動・終了設定が適用されます。[Desktop MCP配信](docs/design/mcp-publish-foundation.md) と
[Hub連携設計](docs/hub-integration.md) を参照してください。
物理Windows複数台・3台再委任の受入は未実施です。Hub再起動をまたぐ再委任経路の復元、Hub/Gatewayのサーバー証明書の
連続稼働中の自動更新、遠隔の対話承認、成果物転送は未対応です。以下のv2.1.1ダウンロードは公開済み版です。
