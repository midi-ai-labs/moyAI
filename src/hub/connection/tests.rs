...
async fn register(
    ...
    (
        ...
        "identity_scope":"server_session","heartbeat_interval_ms":state.heartbeat_interval_ms}),
        ),
    )
}
async fn catalog(
...
async fn waiting_stop_closes_turn_and_disconnected_direct_switch_preserves_review() {
    ...
    let side = service
        ...
        .unwrap()
        .unwrap();
    assert!(service.projection_now().active_side_chat.is_some());
    let client = route.client();
    let mut output = Output::default();
    let result = client.stream_chat(route_request(), cancel.clone(), &mut output);
    let stop = async {
        until(|| {
            server
                .state
    ...
    route.finish().await;
    assert_eq!(
        server.state.closes.lock().unwrap()[0]["path"],
        "/v1/turns/cancel"
    );
    side.finish().await;
    service.shutdown().await;
